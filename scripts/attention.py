...
def log_attention_yaml(log_file: str, message: str) -> None:
...
def ask_check_in(goal: str, log_file: str) -> None:
    play_tone(440.0, 0.3)
    print("\n🔔 How is it going? Please type a quick reflection (or just press Enter):")
    response = input("> ").strip()
    if not response:
        response = "(No input)"
    log_attention_yaml(log_file, response)
    play_tone(880.0, 0.3)
...
def main(goal: str, log_file: str = "attention_log.yaml", interval_min: int = 5) -> None:
    print(f"\n🎯 Goal: {goal}")
    print(f"🕒 You will be prompted every {interval_min} minutes to check in.\nPress Ctrl+C to stop at any time.\n")
    ask_check_in(goal, log_file)
    periodic_check_in(goal, log_file, interval_min * 60)
